...
function VerticalLines() {
  ...
  useEffect(() => {
    ...
    var settings = QuickSettings.create(5, 5, "Settings");
    ...
    function createSettings() {
        ...
        settings
            .addBoolean("Show Points", showPoints, value => {
                pointSymbol.definition.visible = showPoints = value;
            })
            .addBoolean("Lock Y-Axis", yAxisLocked, value => {
                yAxisLocked = value;
            })
            .addBoolean("Moire Effect", showMoire, value => {
                showMoire = value;
            })
            .addRange("Line Spacing", 3, 250, lineSpacing, 1, value => {
                lineSpacing = value;
                debouncedInit();
            })
            .addRange("Path Segments", 2, 25, pathSegments, 1, value => {
                pathSegments = value;
                debouncedInit();
            })
            .addRange("Repel Distance", 5, 400, repelDist, 1, value => {
                repeller.radius = repelDist = value;
                repelDistSq = repelDist ** 2;
                if (repelDist > snapDist) {
                    settings.setRangeValue("Snap Distance", repelDist + 1);
                }
                rasterizeRepeller();
            })
            .addRange("Snap Distance", 5, 400, snapDist, 1, value => {
                snapDist = value;
                snapDistSq = snapDist ** 2;
                if (snapDist < repelDist) {
                    settings.setRangeValue("Repel Distance", snapDist - 1);
                }
            })
            .addRange("Duration", 0.1, 6, duration, 0.1, value => {
                duration = value;
            })
            .addRange("Wave Strength", 1, 2.5, easeStrength, 0.1, value => {
                easeStrength = value;
                elasticEase = Elastic.easeOut.config(easeStrength, easePeriod);
            })
            .addRange("Wave Length", 0.1, 1, easePeriod, 0.1, value => {
                easePeriod = value;
                elasticEase = Elastic.easeOut.config(easeStrength, easePeriod);
            })
            .addDropDown("Path Smoothing", smoothingNames, value => {
    ...
    class ControlPoint {
        ...
        constructor(public index, public line, public path, public start, canMove) {
    ...
    class Line {
        ...
        constructor(public index, public x) {
