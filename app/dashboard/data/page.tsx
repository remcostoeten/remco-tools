...
export default function LeftAside({ className }: LeftAsideProps): JSX.Element {
    ...
    const pi = Math.PI;
    const tau = 2 * pi;
    const map = (value: number, sMin: number | number[], sMax: number | number[], dMin: number, dMax: number) => {
        return dMin + ((value - Number(sMin)) / (Number(sMax) - Number(sMin))) * (dMax - dMin);
    };
        ...
        return (
            <>
                <div className="p-4 h-full">
                    <div className="flex justify-between items-center">
                        <div className="text-white font-bold">Satisfication</div>
                        <div path="res-react-dash-options" className="w-2 h-2" />
                    </div>
                    <div className="mt-3">From all projects</div>
                    <div className="flex justify-center">
                        <animated.svg
                        ...
                        >
                            ...
                            <animated.circle
                                cx={dashOffset.interpolate((x) => 350 + 250 * Math.cos(map(x, [785.4, 0], [Math.PI, 2 * Math.PI])))}
                                cy={dashOffset.interpolate((x) => 350 + 250 * Math.sin(map(x, [785.4, 0], [Math.PI, 2 * Math.PI])))}
                                r={12}
                                fill="#fff"
                            />
                            <circle cx={140} cy={350} r={5} fill="#2f49d0" />
